...
export default function App() {
  ...
  const handleGithubLoad = async (urlToLoad?: string | React.MouseEvent) => {
    ...
    try {
      ...
      // Local Hostinger/Vite static asset fallback
      try {
        const localRes = await fetch('/DASH%20AT1%20PERSONA_ATUALIZADO.xlsx');
        if (localRes.ok) {
          const localBuf = await localRes.arrayBuffer();
          if (localBuf && localBuf.byteLength > 1000 && !cancelImportRef.current) {
            setImportProgress(70);
            processExcelData(localBuf);
            setShowGithubInput(false);
            setGithubUrl('');
            return;
          }
